...
class TimexExchange(ExchangePyBase):
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        qty = Decimal(o_data["order"]["quantity"])
        qty_filled = Decimal(o_data["order"]["filledQuantity"])
        qty_cancelled = Decimal(o_data["order"]["cancelledQuantity"])
        state = tracked_order.current_state
        if qty_filled == qty:
            state = OrderState.FILLED
        elif qty_filled > 0:
            state = OrderState.PARTIALLY_FILLED
        elif qty_cancelled > 0:
            state = OrderState.CANCELED
        order_update = OrderUpdate(
            client_order_id=tracked_order.client_order_id,
            exchange_order_id=str(o_data["order"]["id"]),
            trading_pair=tracked_order.trading_pair,
            update_timestamp=utils.parse_timestamp(o_data["updatedAt"]),
            new_state=state,
        )
        return order_update
    ...
    async def _update_balances(self):
        ...
        for balance in balances:
            ...
            balance_locked = Decimal(balance["lockedBalance"])
            balance_total = Decimal(balance["totalBalance"])
            balance_free = balance_total - balance_locked
            self._account_available_balances[asset] = balance_free
            self._account_balances[asset] = balance_total
            remote_assets.add(asset)
        assets_to_remove = local_assets.difference(remote_assets)
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        if order.exchange_order_id is not None:
            ...
            order_details = await self._api_get(
                path_url=CONSTANTS.ORDER_DETAILS_PATH_URL,
                params={
            ...
            for trade in order_details["trades"]:
                fee = TradeFeeBase.new_spot_fee(
                    fee_schema=self.trade_fee_schema(),
                    trade_type=order.trade_type,
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        ticker = await self._api_request(
